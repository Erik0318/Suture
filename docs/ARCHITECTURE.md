...
1. `app` accepts selected local paths or an optical-drive event.
...
7. `cd` enumerates Linux optical drives through libudev, verifies the disc table of contents, calculates a MusicBrainz ID through libdiscid, retrieves known track titles, and imports or separately exports audio sectors through the bundled cdparanoia sidecar.
...
- FFmpeg, cdparanoia, and curl receive structured argument arrays.
